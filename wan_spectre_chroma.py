...
class Wan_Spectre_Chroma_Filter:
    ...
    FIX: Correction du problème de "Phase Locking" qui causait des vagues de couleurs.
    """
    ...
    def stabilize_spectrum(self, images, stabilization_strength, spectral_smoothness, low_freq_cutoff):
        ...
        print(f"\n\033[96m╔══════════════════════════════════════════════════════════════╗\033[0m")
        print(f"\033[96m║ [XT-SPECTRE-CHROMA] MISSION: SPECTRAL STABILIZATION (FIXED)║\033[0m")
        print(f"\033[96m╚══════════════════════════════════════════════════════════════╝\033[0m")
        
        # 1. PRÉPARATION YUV & RÉFÉRENCE
        # On passe en float32 pour la précision FFT
        ...
        # Masque inversé : On veut stabiliser surtout les basses fréquences chromatiques (fonds unis)
        # mais laisser passer les hautes fréquences si nécessaire pour éviter le flou.
        ...
        for t in range(1, T):
            ...
            # On stabilise UNIQUEMENT la Magnitude (Intensité des couleurs/Structure spectrale)
            # On ne touche PAS à la Phase (Position des couleurs), sinon la couleur se détache de l'objet.
            
            # Interpolation de la magnitude vers la référence pour réduire le flickering
            ...
            # Réassemblage : Luminance originale (Y) + Chrominance stabilisée (UV)
        ...
        print(f"\n   👉 \033[92mSpectral Calibration Finished.\033[0m Phase Locking disabled.\n")
